...
//!
//! Used by ModSync (`Dildz/ModSync-for-SPT4.0`), which is GitHub-only. Fika is a
//! real Forge mod and keeps using the Forge path.
...
pub async fn latest_release(r: &ReleaseRef) -> Result<LatestRelease> {
    ...
    let asset = rel
        ...
        .find(|a| a.name == want)
        .or_else(|| {
            let zips: Vec<&Asset> = rel.assets.iter().filter(|a| a.name.ends_with(".zip")).collect();
            match zips.as_slice() {
                [only] => Some(*only),
                _ => None,
            }
...
/// Unauthenticated GitHub allows 60 requests an hour per IP, and the mods page
/// re-checks on every render, so results are cached per repo. Failures are cached
/// too — an unreachable GitHub must not turn into a request per page view.
///
/// ponytail: one request per GitHub repo per 15 min. Only a handful of mods are
/// GitHub-sourced (the rest come from Forge, which is checked in one batched call),
/// so this stays far under the limit. Revisit only if someone runs dozens of them.
const RELEASE_TTL: std::time::Duration = std::time::Duration::from_secs(900);

type ReleaseCache =
    parking_lot::Mutex<std::collections::HashMap<String, (std::time::Instant, Option<LatestRelease>)>>;
