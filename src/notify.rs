...
//!
//! quma otherwise only checks for updates when someone loads the mods page. This
//! polls on a timer (default 30 min) so admins hear about updates without
//! watching the dashboard, and warms the same cache the page reads, so the page
...
/// One update worth telling someone about.
struct Available {
    mod_db_id: i64,
    name: String,
    current: String,
...
async fn poll_once(
    ...
    // GitHub-sourced mods (ModSync and friends): one call per repo, memoised. Only a
    // handful of mods come from GitHub; the Forge ones were all covered by the single
    // batched call above.
    for m in &installed {
        let Some(r) = m
            .source_url
    ...
    // Announce only what has not been announced before, so a poll every 30 min
    // does not repeat itself until something actually changes.
...
/// SPT orange, so the embeds read as quma's at a glance.
...
/// One embed per update. Mods rarely update in bulk, so a handful of messages is
/// fine and reads better than a single packed list.
...
mod tests {
    ...
    #[test]
    fn each_update_becomes_its_own_embed_naming_both_versions_and_the_source() {
